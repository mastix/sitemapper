...
export default class Sitemapper {
   ...
   * @params {Lastmod} [options.lastmod] - @see {lastmod}
   * @params {UrlFilter} [options.urlFilter] - @see {urlFilter}
   *
  ...
  constructor(options) {
    ...
    this.timeoutTable = {};
    this.lastmod = settings.lastmod;
    if (settings.urlFilter) {
      this.urlFilter = new RegExp(settings.urlFilter, 'gi');
    } else {
      this.urlFilter = null;
    }
  }
  ...
  fetch(url = this.url) {
    ...
    return new Promise((resolve) => this.crawl(url).then((sites) => {
      let filteredSites = sites.filter(element => {
       
        // quickly return if urls do not match
        if (this.urlFilter && element.loc.match(this.urlFilter) === null) {
          return false;
        }

        // quickly return if lastmod is not available
        if (this.lastmod && !element.lastmod) {
          return false;
        }

        // calculate the date in the past and return the element if it's still valid
        if (this.lastmod) {
          return moment(element.lastmod).diff(Date.now(), this.lastmod.measurement) > (this.lastmod.duration * -1);
        }

        // don't check on the lastmod if setting is not used
        return true;
      });
  ...
  parse(url = this.url) {
    ...
    if (url.endsWith('.gz')) {
      return new Promise((resolve) => {
        downloadGzippedSiteMap(url)
          .then(unzipped => xmlParse(unzipped))
      ...
      this.parse(url).then(({ error, data }) => {
        ...
        if (error) {
          ...
          const sites = data.urlset.url.map(site => {
            if (!site || !site.loc || !site.lastmod) {
              return {};
            }

            return {
              loc: site.loc[0],
              lastmod: site.lastmod[0]
            };
          });
